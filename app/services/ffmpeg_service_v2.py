...
Using subprocess with FILE OUTPUT (more reliable than pipe)
"""
import subprocess
import json
import base64
...
class FFmpegService:
    ...
    def generate_thumbnail(
        ...
        """
        Generate thumbnail from video at specified timestamp using FILE OUTPUT
        
        This is more reliable than piping binary data through stdout.
        ...
        try:
            ...
            # Generate thumbnail using FFmpeg (write to file)
            cmd = [
                "ffmpeg",
                "-i", video_path,
                "-ss", str(timestamp),
                ...
                str(thumbnail_path),  # Output to file (NOT stdout)
            ...
            if result.returncode == 0 and thumbnail_path.exists():
                ...
                print(f"✓ Thumbnail generated: {len(thumbnail_base64)} chars, {thumbnail_path.stat().st_size if thumbnail_path.exists() else 0} bytes (file deleted)")
